              ...
              {primes.map(pr => {
                ...
                return (
                  ...
                  >
                    ...
                    <strong className="font-mono">{pr.montant.toLocaleString()} F</strong>
                  </button>
                );
              })}
            </div>
              ...
              {retenues.map(ret => {
                ...
                return (
                  ...
                  >
                    ...
                    <strong className="font-mono text-rose-800">-{ret.montant.toLocaleString()} F</strong>
                  </button>
                );
              })}
            </div>
            ...
            <div className="bg-white border-2 border-slate-400 p-8 rounded-xl shadow-md min-h-[640px] space-y-6 relative overflow-hidden select-text text-xs leading-normal text-slate-800">
              ...
              <div className="flex justify-between items-start border-b-2 border-slate-450 pb-5">
                <div className="text-left space-y-0.5 uppercase tracking-wide">
                  <h4 className="font-bold text-[11px] text-slate-900 leading-tight">RÉPUBLIQUE DE CÔTE D'IVOIRE</h4>
                  <p className="text-[9px] text-slate-600 font-medium">Union • Discipline • Travail</p>
                ...
                <table className="w-full text-left text-[11px] border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-150 text-slate-800 font-mono font-medium">
                    ...
                    <tr className="font-semibold text-slate-950">
                      ...
                      <td className="py-2.5 px-3 text-right font-bold">{baseSalary.toLocaleString("fr-FR")} F</td>
                    ...
                    {activePrimeIds.map(id => {
                      ...
                      return (
                        <tr key={item.id_prime}>
                          ...
                          <td className="py-2.5 px-3 text-right text-emerald-800 font-bold font-mono">+{item.montant.toLocaleString()} F</td>
                          <td className="py-2.5 px-3 text-right text-slate-400">—</td>
                        </tr>
                      );
                    })}
                    ...
                    {customPrimes.map((cp, idx) => (
                      <tr key={idx}>
                        ...
                        <td className="py-2.5 px-3 text-right text-emerald-800 font-bold font-mono">+{cp.amount.toLocaleString()} F</td>
                        <td className="py-2.5 px-3 text-right text-slate-400">—</td>
                      </tr>
                    ))}
                    ...
                    {activeRetenueIds.map(id => {
                      ...
                      return (
                        <tr key={item.id_retenue}>
                          ...
                          <td className="py-2.5 px-3 font-sans text-slate-700">{item.libelle}</td>
                          <td className="py-2.5 px-3 text-right text-slate-400">—</td>
                          <td className="py-2.5 px-3 text-right text-rose-800 font-bold font-mono">-{item.montant.toLocaleString()} F</td>
                        </tr>
                      );
                    })}
                  </tbody>
                ...
                <div>
                  ...
                  <div className="space-y-1 font-mono text-[11px] font-medium text-slate-800">
                    <div className="flex justify-between">
                      ...
                      <strong className="text-slate-900">{(baseSalary + allowancesSum).toLocaleString()} F</strong>
                    </div>
                    <div className="flex justify-between">
                      <span>Total Retenues :</span>
                      <strong className="text-rose-800 font-bold">-{deductionsSum.toLocaleString()} F</strong>
                    </div>
                  </div>
                </div>

                <div className="bg-slate-900 text-slate-100 rounded-xl p-4 text-center font-mono relative flex flex-col justify-center">
                  <span className="text-[9px] uppercase text-slate-400 font-semibold block">Net à Payer (FCFA)</span>
                  <strong className="text-emerald-400 text-2xl font-bold font-mono block tracking-tight mt-1">
                    {computedNetSalary.toLocaleString("fr-FR")} F
                  </strong>

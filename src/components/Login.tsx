...
import {LockKeyhole,User,LoaderCircle,Eye,EyeOff} from 'lucide-react';
...
export default function Login({onAuthenticated}:{onAuthenticated:(user:AuthUser)=>void}){
 const [username,setUsername]=useState('admin');const [password,setPassword]=useState('');const [showPassword,setShowPassword]=useState(false);
 const [error,setError]=useState('');const [loading,setLoading]=useState(false);
 const submit=async(e:React.FormEvent)=>{e.preventDefault();setLoading(true);setError('');try{onAuthenticated(await login(username.trim(),password));}catch{setError('Identifiant ou mot de passe incorrect.');}finally{setLoading(false);}};
 return <main className="min-h-screen bg-[#0A0C10] text-slate-200 grid place-items-center p-6"><div className="w-full max-w-md">
  ...
  <form onSubmit={submit} className="bg-[#161B22] border border-white/10 rounded-2xl p-7 shadow-2xl space-y-5"><div><h2 className="text-lg font-semibold text-white">Accès sécurisé</h2><p className="text-sm text-slate-500 mt-1">Utilisez votre compte professionnel.</p></div>
   <label className="block text-xs font-semibold text-slate-400">Nom d’utilisateur<div className="relative mt-2"><User className="absolute left-3 top-3 text-slate-500" size={17}/><input className="w-full rounded-xl border px-10 py-2.5" value={username} onChange={e=>setUsername(e.target.value)} autoComplete="username" required/></div></label>
   ...
   <button disabled={loading} className="w-full rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-60 text-white font-semibold py-3 flex justify-center gap-2">{loading&&<LoaderCircle className="animate-spin" size={18}/>}Se connecter</button>
  </form></div></main>;
}
